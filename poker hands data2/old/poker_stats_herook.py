...
with open('output/poker_stats.csv', 'w', newline='', encoding='utf-8') as file:
    ...
    # 写入标题行
    writer.writerow(['Player', 'Stat', 'Count', 'Percentage', 'Total', 'Player', 'Stat', 'Count', 'Percentage', 'Total'])
    
    # 写入统计数据
    writer.writerow(['Hero', 'Total Hands', stats['hands'], '100%', stats['hands'], 'Opp', 'Total Hands', stats['hands'], '100%', stats['hands']])
    writer.writerow(['Hero', 'PFR', stats['PFR'], f'{pfr_percentage:.2f}%', stats['hands'], 'Opp', 'PFR', 3416, '68.32%', stats['hands']])
    writer.writerow(['Hero', 'VPIP', stats['VPIP'], f'{vpip_percentage:.2f}%', stats['hands'], 'Opp', 'VPIP', 0, '0.00%', stats['hands']])
    writer.writerow(['Hero', 'ATS', stats['ATS'], f'{ats_percentage:.2f}%', stats['hands'], 'Opp', 'ATS', 0, '0.00%', stats['hands']])
    writer.writerow(['Hero', 'SB Opens', stats['SB_open'], '4.06%', 640, 'Opp', 'SB Opens', 517, '52.01%', 994])
    writer.writerow(['Hero', 'BB Opens', 4, '0.58%', 690, 'Opp', 'BB Opens', 20, '1.79%', 1116])
    writer.writerow(['Hero', 'UTG Opens', stats['UTG_open'], '16.71%', 694, 'Opp', 'UTG Opens', 779, '96.53%', 807])
    writer.writerow(['Hero', 'MP Opens', stats['MP_open'], '16.57%', 682, 'Opp', 'MP Opens', 684, '85.71%', 798])
    writer.writerow(['Hero', 'CO Opens', stats['CO_open'], '18.68%', 680, 'Opp', 'CO Opens', 656, '75.32%', 871])
    writer.writerow(['Hero', 'BTN Opens', stats['BTN_open'], '16.06%', 654, 'Opp', 'BTN Opens', 760, '69.03%', 1101])
    
    # 写入英雄在BB位置面对不同位置加注的跟注频率
    writer.writerow(['Hero', 'BB Call SB Raise', stats['BB_call_SB_raise'], f'{bb_call_sb_percentage:.2f}%', stats['BB_facing_SB_raise'], 'Opp', 'BB Call SB Raise', 0, '0.00%', 0])
    writer.writerow(['Hero', 'BB Call UTG Raise', stats['BB_call_UTG_raise'], f'{bb_call_utg_percentage:.2f}%', stats['BB_facing_UTG_raise'], 'Opp', 'BB Call UTG Raise', 0, '0.00%', 0])
    writer.writerow(['Hero', 'BB Call MP Raise', stats['BB_call_MP_raise'], f'{bb_call_mp_percentage:.2f}%', stats['BB_facing_MP_raise'], 'Opp', 'BB Call MP Raise', 0, '0.00%', 0])
    writer.writerow(['Hero', 'BB Call CO Raise', stats['BB_call_CO_raise'], f'{bb_call_co_percentage:.2f}%', stats['BB_facing_CO_raise'], 'Opp', 'BB Call CO Raise', 0, '0.00%', 0])
    writer.writerow(['Hero', 'BB Call BTN Raise', stats['BB_call_BTN_raise'], f'{bb_call_btn_percentage:.2f}%', stats['BB_facing_BTN_raise'], 'Opp', 'BB Call BTN Raise', 0, '0.00%', 0])
    
    writer.writerow(['Hero', '3B', stats['3B'], f'{threeb_percentage:.2f}%', stats['3B_opportunities'], 'Opp', '3B', 119, '24.24%', 491])
    writer.writerow(['Hero', 'Got 3B', stats['got_3B'], f'{got_3b_percentage:.2f}%', stats['PFR'], 'Opp', 'Got 3B', 83, '2.43%', 3416])
    writer.writerow(['Hero', 'Fold to 3B', stats['fold_to_3B'], f'{fold_to_3b_percentage:.2f}%', stats['facing_3B'], 'Opp', 'Fold to 3B', 0, '0.00%', 83])
    writer.writerow(['Hero', 'Call 3B', stats['call_3B'], f'{call_3b_percentage:.2f}%', stats['facing_3B'], 'Opp', 'Call 3B', 0, '0.00%', 83])
    writer.writerow(['Hero', '4B', stats['4B'], f'{fourb_percentage:.2f}%', 695, 'Opp', '4B', 7, '8.43%', 83])
    writer.writerow(['Hero', 'Got 4B', stats['got_4B'], f'{got_4b_percentage:.2f}%', stats['3B'], 'Opp', 'Got 4B', 1, '0.84%', 119])
    writer.writerow(['Hero', 'Fold to 4B', stats['fold_to_4B'], f'{fold_to_4b_percentage:.2f}%', stats['facing_4B'], 'Opp', 'Fold to 4B', 0, '0.00%', 1])
    writer.writerow(['Hero', 'Call 4B', stats['call_4B'], f'{call_4b_percentage:.2f}%', stats['facing_4B'], 'Opp', 'Call 4B', 0, '0.00%', 1])
    writer.writerow(['Hero', '5B', stats['5B'], '2.04%', 98, 'Opp', '5B', 0, '0.00%', 4])
    writer.writerow(['Hero', 'Got 5B', 0, '0.00%', 4, 'Opp', 'Got 5B', 2, '28.57%', 7])
    writer.writerow(['Hero', 'Fold to 5B', stats['fold_to_5B'], f'{fold_to_5b_percentage:.2f}%', stats['facing_5B'], 'Opp', 'Fold to 5B', 0, '0.00%', 2])
    writer.writerow(['Hero', 'Call 5B', stats['call_5B'], f'{call_5b_percentage:.2f}%', stats['facing_5B'], 'Opp', 'Call 5B', 0, '0.00%', 2])
