...
class PokerHandParser:
    ...
    def _classify_preflop_action(self, action, player_name, player_position):
        ...
        if is_raise:
            ...
            # 处理第二个加注者 (3-bet)
                ...
                if is_hero:
                    self.hand_data['hero_position'] = player_position
                    # 更新hero_3b_position为Hero的位置
                    self.hand_data['hero_3b_position'] = player_position
                else:
                    # 如果不是Hero 3bet，那么Hero不能3bet，设置为NA
                    self.hand_data['hero_3b_position'] = 'NA'
    ...
    def parse_hand(self, hand_text: str) -> Dict:
        ...
        # 确保stakes已设置
        if not self.hand_data['SB_BB'] and self.sb_amount > 0 and self.bb_amount > 0:
            self.hand_data['SB_BB'] = f"(${self.sb_amount}/${self.bb_amount})"
...
def process_poker_file(input_file: str, output_file: str):
    ...
    if all_hands:
        ...
        custom_headers = {
            ...
            'hero_3b_position': 'Hero_3B_Position',
            'pot_type': 'Pot_Type',
