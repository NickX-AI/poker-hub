        ...
        if ':' in line and not line.startswith('Seat'):
            ...
            if self.current_street == 'preflop':
                ...
                if 'folds' in line or bet_amount > 0 or 'checks' in line:
                    ...
                    # OOP玩家第一个行动是check，然后IP玩家下注，认为是IP的Cbet
                    if self.oop_first_action and 'checks' in self.oop_first_action and player_name == self.ip_player and 'bets' in action:
                        self.hand_data['ip_cbet_flop'] = 'YES'
                        self.ip_cbet_done = True
                        
                        # 标记OOP玩家没有下注
                        if not self.oop_cbet_done and not self.oop_donk_done:
                            self.hand_data['oop_cbet_flop'] = 'NO'
                            self.hand_data['miss_cbet_oop'] = 'YES'
                            self.oop_cbet_done = True
                    
                    # OOP玩家先行动且下注(Donk)，标记为不是Cbet
                    elif player_name == self.oop_player and self.oop_donk_done:
                        self.hand_data['oop_cbet_flop'] = 'NO'
                        self.oop_cbet_done = True
                        
                    # OOP玩家check，但IP玩家也check，都没有Cbet
                    elif self.oop_first_action and 'checks' in self.oop_first_action and player_name == self.ip_player and 'checks' in action:
                    ...
                    # 处理对Cbet的反应
                    if self.hand_data['ip_cbet_flop'] == 'YES':
                    ...
                    if self.hand_data['oop_donk_flop'] == 'YES':
                    ...
                    # 处理对Miss Cbet的反应
                    if self.hand_data['miss_cbet_oop'] == 'YES' and self.hand_data['ip_cbet_flop'] != 'YES':
                        if player_name == self.ip_player:
                            if 'bets' in action:
                                self.hand_data['miss_cbet_oop_response'] = 'IP bet'
                            elif 'checks' in action:
                                self.hand_data['miss_cbet_oop_response'] = 'IP check'
                    
                    if self.hand_data['miss_cbet_ip'] == 'YES':
                        if player_name == self.oop_player and not self.oop_first_action:
                            if 'bets' in action:
                                self.hand_data['miss_cbet_ip_response'] = 'OOP bet'
                            elif 'checks' in action:
                                self.hand_data['miss_cbet_ip_response'] = 'OOP check'
