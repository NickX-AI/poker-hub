...
class PokerHandParser:
    ...
    def parse_board_cards(self, line: str):
        if '*** FLOP ***' in line:
            ...
            self.hand_data['preflop_pot'] = self.current_pot  # 记录preflop底池
            cards = re.search(r'\[(.*?)\]', line)
            if cards:
            ...
            self.hand_data['flop_pot'] = self.current_pot  # 记录flop底池
            cards = re.search(r'\] \[(.*?)\]', line)
            if cards:
            ...
            self.hand_data['turn_pot'] = self.current_pot  # 记录turn底池
            cards = re.search(r'\] \[(.*?)\]', line)
            if cards:
    ...
    def parse_action(self, line: str):
        if ':' in line and not line.startswith('Seat'):
            ...
            if 'folds' not in line:
                ...
                bet_amount = self.extract_bet_amount(action)
                self.current_pot += bet_amount
                
                if self.current_street == 'preflop':
                    self.hand_data['preflop_actions'] += action + '; '
                    self.hand_data['preflop_pot'] = self.current_pot
                    print(f"Added preflop action: {action}, pot: {self.current_pot}")
                elif self.current_street == 'flop':
                    self.hand_data['flop_actions'] += action + '; '
                    self.hand_data['flop_pot'] = self.current_pot
                    print(f"Added flop action: {action}, pot: {self.current_pot}")
                elif self.current_street == 'turn':
                    self.hand_data['turn_actions'] += action + '; '
                    self.hand_data['turn_pot'] = self.current_pot
                    print(f"Added turn action: {action}, pot: {self.current_pot}")
                elif self.current_street == 'river':
                    self.hand_data['river_actions'] += action + '; '
                    self.hand_data['river_pot'] = self.current_pot
                    print(f"Added river action: {action}, pot: {self.current_pot}")
    ...
    def parse_result(self, line: str):
        ...
        if 'Total pot' in line:
            ...
            if total_pot_match:
                ...
                self.hand_data['total_pot'] = pot_amount
                print(f"Found total pot: ${pot_amount}")  # Debug info
                
                # 如果到这里 result 还没有设置，说明 Hero 没赢钱
                if not self.hand_data['result']:
                    self.hand_data['result'] = "0"
                    print("Hero didn't win")  # Debug info
        ...
        # 只记录 Hero 赢得的金额
            ...
            if collected_match:
                ...
                self.hand_data['result'] = f"{collected_amount/2:.2f}"
                print(f"Hero won: ${collected_amount/2:.2f}")  # Debug info
