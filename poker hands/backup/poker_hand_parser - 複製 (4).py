...
class PokerHandParser:
    def __init__(self):
        ...
        self.hand_data = {
            ...
            'hero_win': 'NO',
            'hero_win_amount': 0,
            'result': '',  # 添加result键的初始值
            'pot_type': '',  # 底池类型：SRP, 3B, 4B, 5B等
            'final_pot_type': '',  # 最终底池类型：1v1, multi, fold
            'hero_position_type': '',  # Hero的位置类型：IP, OOP, NA
            'preflop_action_1st_bet': '',  # 第一个加注动作
            'preflop_action_3B': '',  # 3bet动作
            'preflop_action_4B': '',  # 4bet动作
            'preflop_action_5B': '',  # 5bet动作
            'preflop_action_limp': '',  # limp动作
            'preflop_action_limp_fold': '',  # limp后fold
            'preflop_action_limp_call': '',  # limp后call
            # Flop 相关字段
            'oop_player': '',  # 记录OOP玩家名称
            'ip_player': '',   # 记录IP玩家名称
            'ip_cbet_flop': 'NO',  # IP位置是否在翻牌圈做了持续下注(基于翻前最后加注者)
            'oop_cbet_flop': 'NO',  # OOP位置是否在翻牌圈做了持续下注(基于翻前最后加注者)
            'ip_cbet_response': '',  # OOP对IP的Cbet的反应(fold/call/raise)
            'oop_cbet_response': '',  # IP对OOP的Cbet的反应(fold/call/raise)
            'oop_donk_flop': 'NO',  # OOP位置是否在翻牌圈做了Donk bet
            'ip_donk_reaction': '',  # IP对OOP的Donk bet的反应(fold/call/raise)
            'miss_cbet_ip': 'NO',  # IP位置玩家是否错过了Cbet机会
            'miss_cbet_oop': 'NO',  # OOP位置玩家是否错过了Cbet机会
            'miss_cbet_ip_response': '',  # OOP对IP Miss Cbet的反应
            'miss_cbet_oop_response': '',  # OOP在TURN的行动 (check/bet)
        }
    ...
    def parse_hero_info(self, line: str):
        if 'Seat' in line and 'Hero' in line:
            ...
            if match:
                ...
                self.hand_data['bb'] = f"{self.convert_to_bb(stack_amount):.0f}BB"
    ...
    def parse_hand(self, hand_text: str) -> Dict:
        ...
        for line in lines:
            ...
            if 'Hold\'em No Limit' in line:
                ...
                if match:
                    ...
                    if self.bb_amount <= 0:  # 防止除零错误
                        self.bb_amount = 0.05
                    self.hand_data['sb'] = f"${self.sb_amount}"
                    self.hand_data['bb'] = f"${self.bb_amount}"
                    break  # 找到盲注信息后立即退出
        ...
        if not self.hand_data['sb'] or not self.hand_data['bb']:
            self.hand_data['sb'] = f"${self.sb_amount}"
            self.hand_data['bb'] = f"${self.bb_amount}"
...
Seat 6: Player6 folded before Flop (didn't bet)"""
    ...
    for key, value in hand_data.items():
        if key in ['ip_cbet_flop', 'oop_cbet_flop', 'ip_cbet_response', 'oop_cbet_response',
                  'oop_donk_flop', 'miss_cbet_oop', 'miss_cbet_oop_response', 
