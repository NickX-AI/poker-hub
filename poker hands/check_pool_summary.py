...
# 打开CSV文件分析Pool_Preflop_Summary内容
with open('parsed_hands.csv', 'r', newline='', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        if pool_summary and pool_summary.strip():
            records_with_value += 1
            unique_values.add(pool_summary)
            value_counter[pool_summary] += 1
    
    # 输出统计信息
    print(f"总记录数: {total_records}")
    print(f"含有Pool_Preflop_Summary值的记录数: {records_with_value}")
    print(f"占比: {records_with_value/total_records*100:.2f}%")
    print(f"唯一值数量: {len(unique_values)}")
    
    # 输出最常见的值
    print("\n最常见的Pool_Preflop_Summary值:")
