...
class PokerHandParser:
    ...
    def parse_result(self, line: str):
        ...
        if 'Total pot' in line:
            ...
            if total_pot_match:
                self.total_pot = float(total_pot_match.group(1))
                print(f"Found total pot: ${self.total_pot}")  # Debug info
                
                # 如果到这里 result 还没有设置，说明 Hero 输了
                if not self.hand_data['result']:
                    self.hand_data['result'] = f"Lost ${self.total_pot/2:.2f} (Pot: ${self.total_pot:.2f})"
                    print(f"Hero lost: ${self.total_pot/2:.2f}")  # Debug info
        ...
        # 记录 Hero 赢得的金额
            ...
            if collected_match:
                ...
                self.hand_data['result'] = f"Won ${collected_amount/2:.2f} (Pot: ${self.total_pot:.2f})"
